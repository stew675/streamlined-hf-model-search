...
- **Generation guard**: All async functions that mutate shared state capture `const gen = _fetchGeneration` at entry and check `if (isStale(gen)) return;` before any side-effect. The queue manager checks generation both at dequeue time and post-fetch. See DESIGN-LOG.md "Architecture Decisions" for rationale.
